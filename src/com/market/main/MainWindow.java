...
public class MainWindow extends JFrame{
		...
		bt6.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (mCart.mCartCount == 0)
					...
					if (mCart.mCartCount == 0)
						JOptionPane.showConfirmDialog(bt6, "장바구니에 항목이 없습니다");
					else if (cartList.mSelectRow == -1)
						JOptionPane.showConfirmDialog(bt6, "장바구니에서 삭제할 항목을 선택하세요");
					else {
				...
				mPagePanel.add("장바구니에 항목 삭제하기", new CartItemListPage(mPagePanel, mCart));
				mPagePanel.revalidate();
				mPagePanel.repaint();
			}
		});
	...
	private void menuCartClear(JButton button) {
		if (mCart.mCartCount == 0)
			...
			if (select == 0) {
				mCart.deleteBook();
				JOptionPane.showConfirmDialog(button, "장바구니의 모든 항목을 삭제했습니다");
			}
		}
	}
}
